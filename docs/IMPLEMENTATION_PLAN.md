...
- [x] Исправление (2026-09-21), TC-GRAPH-COLOR-02: в точке `811563a` общая
  история `main` остаётся синей после слияния `dev`, ответвление `dev` — зелёным.
  Цвет продолжается через первого родителя merge; при схождении `main/master`
  имеют приоритет над `dev/develop`. Именованные родители сохраняют свой цвет.
  Добавлены 17 проверок Core/UI: 12 воспроизвели дефект до исправления, одна
  защищает именованного родителя за промежуточным коммитом, четыре проверяют
  настоящий no-ff merge и пиксели синей вертикали/зелёного ответвления.
  Целевой набор графа Core/ViewModel/UI — 332 проверенных сценария; полный
  pytest не запускался. Ruff `src/ tests/` и `git diff --check` без ошибок.
  На реальном `git-py` (только чтение) проверены `811563a`, предки и коммиты
  `dev`, сохранён offscreen-снимок. 5000 коммитов с merge: 26,4 мс без WIP,
  29,6 мс с WIP. Git-тесты запускались вне песочницы из-за доступа к временным `.git`.
- [x] Исправление (2026-09-21), TC-GRAPH-WIP-02: WIP продолжает колонку HEAD
  через `HORIZONTAL_PIPE`, сохраняя обе линии и цвета пересечения. Верхний
  down-bend дополняется продолжением вверх. Обход свободной колонкой проверяет
  все строки, включая HEAD, и не заменяет соседние вертикали горизонталью.
  Добавлены 14 регрессий Core/ViewModel/UI: топология двух merge из `math-portal`,
  detached HEAD, соседние stash/WIP, octopus merge, пиксельная связность и
  возврат к чистому графу. Сложный двухцветный relay-bend не упрощается до TEE.
  Расширенный прогон — **337 passed** (173,60 с, включая 5 проверок R3.1);
  после упрощения кода обхода и добавления octopus-кейса повтор без R3.1 —
  **333 passed** (36,51 с). После защиты relay-bend — **133 passed** (1,26 с).
  Ruff `src/ tests/` и `git diff --check` без ошибок; полный pytest не запускался.
  На данных исходного worktree `math-portal` (только чтение) WIP с 12 изменениями
  и HEAD `79f6fd60` имеют колонку 1, соседняя вертикаль сохранена. Проверен
  offscreen-снимок. Синтетические 5000 коммитов: 17,3 мс без WIP, 20,8 мс с WIP.
- [x] Регрессия (2026-09-20), TC-GRAPH-COLOR-01: сквозной сценарий через
  `CommandProcessor` и `GraphViewModel` — 10 коммитов `dev`, создание/checkout
  `redesign`, возврат на `dev`, два новых коммита, переоткрытие и первый
  собственный коммит `redesign`. После каждого шага проверяется полная карта
  SHA → цвет и HEAD. Старый алгоритм, загруженный только в память процесса,
  воспроизводимо проваливает тест после первого нового коммита `dev`.
  Тест и пошаговый кейс добавлены в `tests/viewmodels/test_branch_color_workflow.py`
  и `TEST_PLAN.md`; запускаются существующим CI. Целевой набор цветов
  Core/ViewModel/UI — **34 passed**, 1,61 с; Ruff `src/ tests/` без ошибок.
- [x] Исправление (2026-09-20): новая ветка на старом tip больше не меняет
  цвет продолжающейся истории (`agents-ide`: `dev` → `redesign` → новый
  коммит на `dev`). Именованная цепочка передаёт цвет независимо от колонок,
  stash/WIP и HEAD; плашки получают собственный цвет по имени, как popup.
  Добавлены 33 регрессии Core/UI: обычное имя ветки, текущие/отстающие remote
  refs, checkout, расхождение веток, слияния и пиксели плашек.
  Финальный целевой прогон графа Core/ViewModel/UI — **300 passed**, 27,49 с
  (Qt offscreen). До последнего уточнения выбора родительского ref также
  прошли 5 проверок `test_graph_v2_r3_1.py`; полный pytest не запускался.
  Ruff `src/ tests/` и `git diff --check` без ошибок. На реальном `agents-ide`
  (только чтение) проверены зелёная история от `dev` через `d116ecb`, отдельная
  плашка `redesign` и синяя граница `main`. Синтетические 5000 коммитов + stash:
  20,1 мс без WIP, 21,8 мс с WIP. Тесты с Git запускались вне песочницы из-за
  ограничений доступа к временным репозиториям.
- [x] Дополнение (2026-09-20): контурное дерево слева от имени ветки соседнего
  worktree на графе и в списке `BranchStackPopup`; строка popup показывает путь
  в подсказке. Метка есть и для чистой папки, не дублируется на remote/current
  и снимается при detached HEAD. Снимок HEAD обновляется фоновым монитором,
  включая невидимые ветки за лимитом истории; постоянные тики не перестраивают
  граф. Размер учитывается в прокрутке, переход во вкладку сохраняется.
  Проверена отрисовка на снимке графа `math-portal` (репозиторий только читался).
  Целевые проверки Core/ViewModel/UI графа, popup, worktree, checkout и async —
  **187 passed** (Qt offscreen, 46,02 с), включая 4 новых сценария. Ruff
  и `git diff --check` без ошибок; полный pytest не запускался.
- [x] Исправление (2026-09-19): связность stash после очистки worktree.
  Вместо переноса готовых узлов граф заранее резервирует колонку HEAD только
  при наличии WIP; линии stash строятся общим алгоритмом. Добавлены 11
  регрессионных сценариев Core, ViewModel и пиксельной отрисовки; новые тесты
  воспроизводят дефект на прежнем алгоритме. Целевой набор графа и worktree:
  **279 passed**, два старых UI-теста потребовали обновления подготовки
  геометрии; после обновления оба прошли (**2 passed**, без изменения проверок
  отсутствия лишних линий). Всего проверен 281 сценарий; полный pytest не запускался.
  Read-only проверка исходного worktree подтвердила непрерывную связь stash с
  `811563a`. Синтетические 5000 коммитов + stash: 23 мс без WIP, 27 мс с WIP.
...
- [x] Исправление (2026-09-19): пустой diff показывает пояснение вместо пустой
  панели. Воспроизведено на `agents-ide/backend/src/agents_ide/domain/graph_preflight.py`:
  HEAD/index содержат LF, рабочий файл — CRLF, при `core.autocrlf=true` патч пуст.
  Пояснение не становится текстом diff или строкой для staging. Регрессии
  проверяют клик по файлу в обоих режимах, завершение загрузки и сохранность
  HEAD, индекса и рабочего файла. Целевой прогон — **67 passed** (Qt offscreen):
  `tests/ui/test_diff_loading.py`, `tests/ui/test_diff_view_widget.py`,
  `tests/viewmodels/test_file_diff_async.py`, `tests/core/test_file_diff.py`.
  Тесты Git выполнены вне песочницы из-за отказа libgit2 при создании временных `.git`.
- [x] Оптимизация (2026-09-19): чтение статуса через отдельный Git-процесс
  устраняет удержание GIL при опросе worktree. Индекс не записывается,
  граф повторно использует снимок панели; после сканирования выдерживается
  интервал опроса. Подсчёт размера merge читает deltas без генерации патчей.
  На math-portal максимальная пауза Python снизилась с 1,7–1,8 с до 18–23 мс;
  offscreen-граф с кликами и мониторингом — максимум 44 мс между событиями.
  Целевой прогон — **492 passed**, 88,12 с; после добавления регрессий junction
  и подсчёта merge — **35 passed**, 2,55 с. Полный pytest не запускался.
  Методика, ограничения и команды: [PERFORMANCE_WORKTREES.md](PERFORMANCE_WORKTREES.md).
- [x] Исправление (2026-09-19): крестик закрывает свою вкладку, включая открытый
  соседний worktree. Локальные координаты кнопки переводятся в координаты панели;
  перетаскивание синхронизирует порядок вкладок с ViewModel и сохраняет активный
  репозиторий. Добавлены 19 регрессионных сценариев. Целевой прогон вкладок,
  worktree, MainWindow, сохранения окна, настроек и Qt lifecycle — **109 passed**
  (Qt offscreen, 4,31 с); Ruff и `git diff --check` без ошибок.
  Команда: `python -m pytest tests/ui/test_repo_tab_close.py
  tests/ui/test_repo_bar_widget.py tests/viewmodels/test_repo_tabs_viewmodel.py
  tests/ui/test_other_worktrees.py tests/ui/test_main_window.py
  tests/ui/test_window_persistence.py tests/ui/test_settings_audit.py
  tests/ui/test_qt_lifecycle_r2_6.py -q --tb=short -p no:cacheprovider`.
  Для `--basetemp` использован отдельный временный каталог; Git-тесты запущены
  вне песочницы из-за отказа libgit2 при создании `.git` внутри неё.
- [x] Дополнение (2026-09-19): WIP-узлы других worktree с привязкой к их HEAD,
  фоновое обновление и открытие рабочей папки в новой вкладке по кнопке узла
  или из контекстного меню. Правая панель показывает ветку, путь и счётчик;
  открытая вкладка используется повторно. Поддержаны основная папка из связанной,
  detached HEAD, несколько worktree на одном коммите и недоступные рабочие папки.
  Целевой прогон Core/ViewModel/UI графа, вкладок и мониторинга — **416 passed**
  (Qt offscreen, 178,40 с), включая 16 новых тестов. После последних правок
  новые тесты повторены: **16 passed**. Ruff и `git diff --check` без ошибок.
  На указанной ветке `math-portal` обнаружены 20 изменённых файлов; интерфейс
  проверен offscreen-снимком. Контрольные суммы HEAD и индексов до/после чтения
  совпали. Тесты выполнялись вне песочницы из-за доступа к временному каталогу.
- [x] Дополнение (2026-09-12): режим редактирования рабочих файлов из Staged/Unstaged,
  Save/Cancel, Ctrl+S, автосохранение при переходах, локальные Undo/Redo текста
  и отменяемое сохранение через CommandProcessor. Индекс сохраняется; при ошибке
  записи переход блокируется, черновик остаётся в редакторе. Фоновый мониторинг
  не заменяет текст, а устаревшие ответы чтения отбрасываются. Сохраняются BOM,
  кодировка, окончания строк и отсутствие завершающего перевода строки.
  Проверки: **388 passed** (Qt offscreen), `ruff check src/ tests/ --no-cache`
  и `git diff --check` без ошибок. Прогон: `tests/core/test_file_edit.py`,
  `tests/viewmodels/test_file_editor.py`, `tests/ui/test_file_editor.py`,
  `tests/ui/test_diff_view_widget.py`, `tests/ui/test_commit_panel.py`,
  `tests/ui/test_right_panel.py`, `tests/ui/test_main_window_toolbar.py`,
  `tests/ui/test_window_persistence.py`, `tests/ui/test_worktree_refresh.py`,
  `tests/viewmodels/test_commit_panel_viewmodel.py`,
  `tests/viewmodels/test_worktree_refresh.py`, `tests/ui/test_ignore.py`,
  `tests/utils/test_config.py`. В новом UI-тесте порядок завершения фоновых задач
  уточнён после ошибки teardown; итоговый прогон прошёл полностью.
- [x] Исправление (2026-09-12): Ignore каталога сразу убирает его untracked-файлы
  из Unstaged Files, включая подпапки. Новое правило отделяется переводом строки
  даже при его отсутствии в конце `.gitignore`; пункт каталога уровнем выше
  использует соответствующий путь, разделители Windows нормализуются в `/`.
  Регрессии проверяют настоящее меню, счётчик, сброс выбранного diff, Undo/Redo,
  сохранность файлов и индекса: `tests/core/test_gitignore.py`, `tests/ui/test_ignore.py`.
  Вместе с `tests/core/test_operations.py`,
  `tests/viewmodels/test_commit_panel_viewmodel.py`,
  `tests/viewmodels/test_worktree_refresh.py`, `tests/ui/test_commit_panel.py`
  и `tests/ui/test_worktree_refresh.py` — **235 passed** (Qt offscreen),
  включая 19 новых тестов. `ruff check src/ tests/ --no-cache` и `git diff --check`
  без ошибок. Проверки выполнялись вне песочницы из-за отказа libgit2
  в доступе к временным Git-репозиториям внутри неё.
- [x] Исправление (2026-09-12): независимый фоновый мониторинг рабочей папки,
  отображение файла одновременно в Staged и Unstaged после сохранения в редакторе,
  обновление выбранного diff и повтор запроса обновления по фокусу после busy.
  Индекс автоматически не изменяется. Проверены жизненный цикл, устаревшие
  результаты, сохранение черновика/Undo и регрессии соседних сценариев.
  Целевой прогон — **181 passed** (Qt offscreen), Ruff и `git diff --check`
  без ошибок. До уточнения ожидания сигнала в новом тесте расширенный прогон
  дал 824 passed / 1 failed. В одном повторе также возник WinError 5 в старом
  тесте одновременного сохранения настроек; отдельно и в итоговом прогоне он прошёл.
- [x] Исправление (2026-09-12): обычный коммит использует подпись автора также
  для коммиттера, если отдельный коммиттер не передан явно. Перед staging файла
  и созданием коммита индекс перечитывается с диска, чтобы внешние Git-операции
  и фоновое обновление UI не приводили к записи устаревшего дерева.
  В `math-portal` дерево `d7122b9` совпало с `a819a91` — были фактически отменены
  изменения восьми последующих коммитов до `5099ba8`. `64ee2ab` вернул их вместе
  с новыми изменениями (суммарный diff `5099ba8..64ee2ab`: 41 файл, +3944/-19).
  Генерация сообщения не меняет индекс; регрессия воспроизведена с ней и без неё
  на временных репозиториях. История `math-portal` исследована только чтением.
  Проверки: **274 passed**, Qt offscreen, `ruff check src/ tests/ --no-cache`
  и `git diff --check` без ошибок. Прогон:
  `python -m pytest tests/core/test_operations.py tests/core/test_staged_diff.py
  tests/viewmodels/test_commit_command.py tests/viewmodels/test_commit_panel_viewmodel.py
  tests/viewmodels/test_ai_generation.py tests/ui/test_ai_workflow.py
  tests/ui/test_commit_panel.py -q --tb=short -p no:cacheprovider`.
...
- [x] Дополнение (2026-09-06): просмотр изображений по клику в правой панели —
  worktree/index/commit/stash, версия до удаления, фоновое декодирование,
  Fit/100%/масштаб и перемещение. Проверки: `test_file_content.py`,
  `test_image_preview.py`, `test_image_view_widget.py`.
  Итоговый целевой прогон core/repository, просмотрщика, правой панели,
  текстового diff, сохранения окна и связанных ViewModel — **285 passed**
  (17,52 с, Qt offscreen), включая 24 новых теста. `ruff check src/ tests/`
  и `git diff --check` — без ошибок. Временные Git-репозитории создавались вне
  рабочей копии, чтобы проверка пустой папки не находила родительский `.git`.

## Этап 4: Работа с ветками и переключение
- [x] Исправление (2026-09-20): Checkout локальной ветки со слешами больше
  не принимает первый сегмент за remote. Двойной клик и меню графа/левой панели
  переходят во вкладку существующего worktree; если вкладки нет, она создаётся.
  Поиск по свежему HEAD включает чистые папки и основную рабочую копию,
  исключает detached HEAD и не читает индексы. Переключение вкладок сохраняет
  незакоммиченные изменения, обычный checkout по-прежнему поддерживает Undo.
  Добавлены 15 сценариев Core/ViewModel/UI; проверка на `math-portal` только
  на чтение подтвердила путь worktree ветки `agents-ide/run/65a1b4daa37a4972a6c0e583da6881a2`.
  Целевой прогон веток, worktree, вкладок, графа и связанных команд —
  **319 passed** (Qt offscreen, 59,65 с); Ruff и `git diff --check` без ошибок.
  Тесты выполнялись во временных репозиториях вне песочницы Windows;
  полный pytest не запускался.
- [x] Дополнение (2026-09-08): `Create tag here…` в меню коммита и
  локальной/удалённой ветки на графе. Диалог имени создаёт lightweight-тег
  на SHA выбранной строки через `CreateTagCommand`; граф и левая панель
  обновляются, Undo/Redo работают. Исправлены устаревшие вызовы pygit2 для
  обычных и annotated-тегов; Undo сохраняет тег, заменённый внешней операцией.
  **297 passed** в целевом прогоне core/repository, команд, графа, MainWindow
  и LeftPanel, включая 36 новых тестов. Qt offscreen; временные Git-репозитории
  создавались вне песочницы, где pytest недоступен общий временный каталог.
...
- [x] Исправление (2026-09-23): ложная коллизия revert с игнорируемой папкой
  `__pycache__` при отмене `9813fdf` в worktree `agent-skills`. libgit2 сворачивает
  ignored-каталог в одну запись статуса: теперь проверяются конкретные пути
  назначения и их предки, а посторонние файлы внутри каталога сохраняются.
  Общая проверка применяется также к Undo/Redo; символические ссылки и Windows
  reparse points не обходятся. Четыре новых сценария воспроизводили отказ до
  исправления; целевой набор — **109 passed**, Ruff и `git diff --check` без ошибок.
  Проверка путей на пользовательском worktree прошла только на чтение; сам revert
  там не выполнялся. Полный pytest не запускался.
- [x] Дополнение (2026-09-23): Revert commit из контекстного меню графа
  создаёт обратный коммит в текущей ветке; для merge есть выбор родителя.
  Выполнение, Continue/Abort и Undo/Redo проходят через команды и фоновый
  исполнитель. Поддержаны конфликты, переоткрытие репозитория, подпись из
  настроек и linked worktree. Локальные изменения и коллизии с untracked/
  ignored-файлами защищены; Undo/Redo проверяют внешнее изменение HEAD и файлов.
  Добавлены 44 сценария Core/ViewModel/UI. Расширенные регрессии — **373 passed**;
  финальная проверка revert и редактора конфликтов — **56 passed** (Qt offscreen).
  Ruff `src/ tests/` и `git diff --check` без ошибок. Полный pytest не запускался;
  тесты Git запускались вне песочницы из-за доступа к временным репозиториям.
- [x] Исправление (2026-09-21): убран локальный лимит входного запроса при
  AI-разрешении конфликтов; допустимый размер определяет API провайдера.
  Проверена полная передача больших конфликтов и пользовательского контекста,
  успешный ответ и отказ HTTP 413. Целевые utils/ViewModel тесты — **101 passed**;
  Ruff и `git diff --check` без ошибок.
- [x] Исправление (2026-09-19): Continue, нажатая во время обновления при возврате
  в окно, выполняется один раз после успешной загрузки вместо ошибки busy.
  Ожидающий запрос привязан к репозиторию, HEAD и операции; ошибка загрузки,
  смена репозитория или внешнее завершение merge отменяют его. Реальная Git-команда
  по-прежнему защищена от повторного запуска. `busy` загрузки снимается только
  после `lifespan_finished`, без вложенной обработки событий в результате.
  Добавлены 8 сценариев в `tests/ui/test_conflict_workflow.py`: merge/rebase,
  повторные клики/обновления, ошибка и повторная попытка, смена репозитория,
  внешнее завершение, оставшиеся конфликты и блокировка при Git-операции.
  Целевой прогон конфликтов, async, lifecycle, мониторинга и MainWindow —
  **173 passed** (Qt offscreen, 33,36 с); дополнительный сценарий отложенного
  rebase — **1 passed** (0,91 с). Ruff без ошибок. Проверки выполнялись
  вне песочницы Windows во временных Git-репозиториях; полный pytest не запускался.
- [x] Исправление (2026-09-19): Resolve открывает редактор при нескольких
  конфликтах без предварительного выбора файла. Первый файл выбран автоматически;
  обновление списка сохраняет выбор по пути или выбирает первый оставшийся файл.
  Добавлены 8 регрессионных сценариев в `tests/ui/test_conflict_workflow.py`,
  включая реальный клик по кнопке и открытие модального диалога. До исправления
  тест открытия без выбора воспроизводил ошибку. Целевой прогон конфликтов
  Core/ViewModel/UI — **118 passed** (Qt offscreen, 12,34 с); Ruff без ошибок.
  Тесты запускались вне песочницы из-за запрета записи во временные каталоги.
- [x] Исправление (2026-09-19): fast-forward с `no_ff=True` обновляет индекс
  и рабочие файлы через SAFE checkout до создания merge-коммита. Воспроизведено
  по `math-portal`, коммит `3f06a6fd`: все 34 staged-файла совпадали с версиями
  до merge, тогда как дерево коммита уже содержало результат слияния.
  Добавлены 9 регрессионных сценариев: текущая/другая целевая ветка,
  изменение/удаление/добавление файлов, отказ при пересекающихся локальных
  правках до перемещения HEAD и сохранение посторонних staged/unstaged-правок.
  Целевые проверки — **245 passed** (два прогона: 190 + 55, Qt offscreen):
  `tests/core/test_merge_no_ff.py`, `tests/core/test_operations.py`,
  `tests/core/test_r1_1_merge_mid_state.py`, `tests/viewmodels/test_commands.py`,
  `tests/viewmodels/test_review_followup.py`, `tests/viewmodels/test_merge_commands.py`,
  `tests/viewmodels/test_main_viewmodel_merge.py`. Ruff — без ошибок.
  Полный pytest не запускался; `math-portal` проверялся только на чтение.
- [x] Доработка (2026-09-18–19): две сопоставленные стороны конфликта, выбор строк
  через +/− и чекбоксы с сохранением порядка; результат снизу; AI по текущим
  настройкам модели, общий редактируемый промпт и пользовательский контекст.
  Независимые изменения объединяются автоматически, порядок кликов сохраняется
  внутри каждого конфликтующего участка. Промпт и контекст общие для файлов;
  запрос разрешает все участки открытого файла и возвращает проверяемый черновик.
  Сохранение через ResolveConflictCommand проверяет HEAD, стадии индекса и файл
  против исходного снимка. Undo остаётся у завершающей merge/rebase-команды.
  **316 passed** (Qt offscreen, 21 с), Ruff и `git diff --check` без ошибок.
  Проверены core/VM/UI нового редактора, существующие merge/rebase/Undo/Redo,
  AI HTTP-клиент с локальным сервером, настройки, генерация сообщений и diff.
  На `math-portal/docs/implementation-status.md`: один конфликт 83/109 строк,
  192 строки при выборе обеих сторон; AI-контекст 21 975 символов. Диалог
  проверен снимком; SHA-256 файла, HEAD и индекса до/после совпадают. Внешняя
  модель при проверке не вызывалась, слияние в math-portal не завершалось.
...
- Визуальный редактор конфликтов: целевая и вливаемая версии рядом, цветовое
  сопоставление строк и символов, +/− и чекбоксы, редактируемый результат снизу.
  Base используется для трёхстороннего сравнения внутри Core.
- AI-разрешение конфликтов добавлено в доработке выше; используются настройки
  существующего AI-клиента, фоновые запросы и явное сохранение результата.
...
- [x] Доработка (2026-09-19): понятное сообщение о занятом файле/каталоге при
  создании stash, различение сохранённого архива и незавершённой очистки,
  обновление панелей после частичного сбоя. Проверены реальные блокировки файла
  и каталога Windows, старый stash при отказе до сохранения, три VM-входа и
  уведомление по кнопке тулбара. **77 passed**, включая 14 новых проверок;
  Ruff и `git diff --check` без ошибок. В группе 50 существующих stash-тестов
  после успешного прогона возникает `RuntimeError: Signal source has been deleted`
  при завершении фонового QRunnable; воспроизводится отдельно без новых тестов.
  Новый UI-тест отдельно завершается без этой ошибки. Полный pytest не запускался.
...
- [x] Доработка (2026-09-19): вкладки промптов коммита/конфликтов, языковые
  варианты инструкции для конфликтов и выбор добавления ветки в summary/description.
  Добавлены семь языков инструкции, восстановление шаблона, локальные черновики
  при переключении языков, сохранение текущего промпта для AI-разрешения.
  Название ветки добавляется приложением в отмеченные поля как `[имя-ветки]`.
  Исправлен воспроизведённый WinError 5 при одновременной замене JSON-настроек:
  временные файлы готовятся независимо, замена выполняется последовательно.
  Итоговый целевой прогон — **220 passed** (Qt offscreen, 11,97 с), включая
  проверки AI-настроек, генерации, разрешения конфликтов и сохранения JSON.
  Ruff и `git diff --check` — без ошибок; обе вкладки проверены снимками.
- [x] ИИ: генерация summary/description по staged-диффу, настройки подключения,
  загрузка моделей, тест LLM и редактор промпта с языком и пресетами (2026-09-06).
...
## Этап 10: Тестирование и стабилизация — _завершено (R4, 2026-07-21)_
- [x] Исправление блокировок репозитория в Windows (2026-09-14): освобождение
  pack-файлов libgit2 в простое и при закрытии менеджеров; запуск оболочки
  терминала только при вводе команды. До исправления воспроизведён `WinError 32`
  для pack-файла после чтения истории и каталога при скрытом терминале.
  Регрессии проверяют внешний commit/repack/fsck, rename каталога при открытом
  окне, sync/async загрузку, сохранность индекса и освобождение при traceback.
  Целевой прогон — **157 passed**, ViewModel — **510 passed**;
  итоговый `python -m pytest -q` — **1734 passed** (Windows, Qt offscreen).
  В первом полном прогоне два теста мониторинга считали запросы посторонних
  ViewModel: подмена чтения ограничена проверяемым репозиторием с сохранением
  проверки повторного запуска. `ruff check src/ tests/ --no-cache` и
  `git diff --check` — без ошибок. Тесты запускались вне песочницы из-за
  отказа libgit2 в доступе к временным репозиториям внутри неё.
- [x] Дополнительная стабилизация диффа (2026-09-06): фоновая загрузка WIP/staged/
  Full document, ограничение очереди чтений, индикатор после 150 мс,
  вычисление только выбранного патча и защита от сверхдлинных строк.
  Целевые проверки — **200 passed**, Ruff и `git diff --check` без ошибок.
- [x] Покрытие UI-тестами основного workflow: открытие репо, создание коммита, переключение веток, слияние без конфликтов.
- [x] Стресс-тесты на больших репозиториях (1000+ коммитов) для проверки производительности графа.
- [x] Исправление багов, оптимизация отрисовки графа (кэширование раскладки, виртуализация).
- [x] Регрессионные тесты R1–R4 и проверка `ruff check src/ tests/`.
...
### Чек-лист этапов
- [x] **Дополнение к этапу 3 (2026-09-23): снятие отслеживания ignored-файлов** —
  `Stop Tracking (Keep Local File)` в ПКМ для одного/нескольких файлов,
  сохранение рабочих копий, фоновое выполнение и Undo/Redo с проверкой HEAD/индекса.
  Core, ViewModel, UI и смежные регрессии — **141 passed**;
  `ruff check src/ tests/ --no-cache` и `git diff --check` — без ошибок.
...
- [~] **Этап 9: Конфигурация и темизация** — _в работе (под-этапы: темизация ✓, персистентность окна ✓)_
  - [x] Исправления аудита настроек (2026-09-08): безопасный отказ
    Discard при превышении лимита копии, валидация JSON, применение параметров
    из одного config_path, колонки графа по репозиториям и обработка отказа записи.
    Уникальные временные файлы устраняют коллизии параллельного сохранения.
    Добавлены 42 регрессионных сценария; старый тест auto-fetch теперь проверяет
    реальный tick и останавливает таймер, чтобы не влиять на последующие тесты.
    Проверен весь набор из 1625 тестов: 1609 passed в полном прогоне вне песочницы,
    ещё 16 passed при повторе в коротком временном каталоге Windows (длинные пути
    первоначально вызывали ошибки Git CLI). Ruff и `git diff --check` без ошибок.
  - [x] Таймаут SSH push (2026-09-08): 30 минут по умолчанию вместо 60 секунд,
    `push_timeout_seconds` (1–86400 секунд), поле в Settings → General / SSH,
    применение к следующему push без перезапуска. Лимит передаётся через
    PushCommand в Core; прочие транспорты pygit2 не меняются.
    Проверены 300 целевых тестов: 298 в основном прогоне и 2 после исправления
    обращения к методу в новом тесте. Включены 20 новых регрессионных случаев;
    Git-репозитории временные, SSH-вызовы заменены заглушками. Ruff и
    `git diff --check` без ошибок. Для Git-тестов потребовался запуск вне
    песочницы, запрещающей создание тестовых `.git`-каталогов.
  - [x] Компактный контекст ИИ-коммитов (2026-09-08): метаданные для удалений,
    переносов без правок, изображений/RAW/бинарных ресурсов; полный список операций
    резервируется до добавления патчей. Текстовые правки включаются в пределах
    `ai.max_diff_chars`, пропуски помечаются явно. **105 passed** в целевых core,
    utils, ViewModel и UI тестах; Ruff и `git diff --check` без ошибок.
  - [x] ИИ-сообщения коммитов (2026-09-06): звёздочки в Commit Summary,
    фоновая генерация обоих полей по HEAD → index, вкладка AI с URL/ключом/
    провайдером/моделью, получение моделей по клику, тест LLM, редактор промпта,
    язык и четыре пресета. Защита от устаревших ответов и сохранение настроек.
    Инструкция и ограничения: [AI_COMMIT_MESSAGES.md](AI_COMMIT_MESSAGES.md).
    Проверки: **60 новых тестов**, **214 passed** в целевой выборке; полный набор
    покрыт двумя частями — **333 core + 1108 остальных = 1441 passed**.
    Первый полный запуск остановился на проверке владельца рабочей копии;
    повторные тесты использовали отдельную временную Git-конфигурацию с точным
    `safe.directory` проекта и тестовым автором. Пользовательский Git config не менялся.
    Ruff и `git diff --check` без ошибок; внешние LLM не вызывались.
    Дополнение 2026-09-06: исправлен разбор ответа MiniMax с начальным
    `<think>…</think>` перед JSON; 7 новых регрессионных случаев, **60 passed**
    в целевых ИИ-тестах. Воспроизведение и успешная повторная генерация проверены
    на настроенной MiniMax-M3 с искусственным диффом; данные проекта не передавались.
    Повторная диагностика с разрешения пользователя на staged-диффе 97 067 символов:
    исправлен режим ответа MiniMax-M3 и разбор итогового JSON после пояснений;
    добавлены конкретные ошибки формата. **77 passed**, 17 новых случаев, Ruff и
    `git diff --check` без ошибок. Живая генерация обоих полей прошла, HEAD/index
    не изменились; подробности в `AI_COMMIT_MESSAGES.md`.
...
- [x] **Этап 10: Тестирование и стабилизация** — _завершено (R4, 2026-07-21)_
  - Дата начала: `2026-06-08`
  - Дата завершения: `2026-07-21`
  - Комментарий: финальный проход update1 (стадии R1–R4): UI-тесты основного workflow, стресс-тесты на больших репах, регрессионные тесты R1–R4, чистый `ruff check src/ tests/`. Сводный план см. `docs/updates/update1/STATUS-R4.md`.

### Текущий статус
- **Активный этап:** `Этап 10: Тестирование и стабилизация` — _завершён (R4, 2026-07-21)_
- **Версия:** `v0.13.0` (ИИ-сообщения коммитов, просмотр изображений, фоновая загрузка diff и настройка таймаута SSH push)
- **Последнее обновление:** `2026-09-08`
- **Прогресс:** `9 / 11` этапов завершено (Этапы 0–8 и 10 ✓; Этап 9 — в работе: темизация ✓, персистентность окна ✓, осталось Settings-диалог и импорт/экспорт настроек).
- **Следующий шаг:** `Завершить Settings-диалог (переключатель dark/light), импорт/экспорт настроек (Этап 9).`
...
### Проверка ускорения диффа (2026-09-06)

- WIP/staged/Full document загружаются в фоне; коммиты/stash используют тот же
  ограниченный исполнитель LatestWorker. Каждый воркер открывает собственный
  RepositoryManager. Смена файла/репозитория/закрытие окна инвалидируют результаты
  и ошибки, не ожидая завершения потока.
- Для одного файла перебираются метаданные Diff.deltas, а не все патчи.
  Проверка бинарного формата читает 8 КБ вместо всего файла.
  MainWindow подписан только на diff_pair_ready, без повторной отрисовки
  по diff_ready. Индикатор появляется через 150 мс. Ограничения отображения
  больших диффов явно обозначаются; полный исходный текст сохраняется.
- Синтетический замер: 150 файлов по 3000 изменённых строк и один маленький
  выбранный файл. Три запуска старого извлечения всех патчей:
  **0,3968 / 0,3366 / 0,2329 с**; извлечение только выбранного:
  **0,0016 / 0,0020 / 0,0011 с**. Это измерение данного сценария, а не
  гарантия скорости на любом репозитории.
- Offscreen-проверки: python -m pytest для test_file_diff.py,
  test_r4_cleanup.py, test_commit_panel_viewmodel.py,
  test_commit_detail_async.py, test_file_diff_async.py,
  test_diff_view_widget.py, test_right_panel.py, test_diff_loading.py
  и test_qt_lifecycle_r2_6.py — **200 passed, 11,33 с**.
  Ruff — **All checks passed**; git diff --check — без ошибок.
  Индикатор также проверен на снимке MainWindow с тёмной темой.
- Полный прогон не полностью зелёный: **1503 passed, 3 failed, 1 error,
  254,73 с**. В test_initial_commit_uses_index_blobs получен повреждённый
  текст из существующего staged_diff.py; два варианта
  test_async_remote_checkout_targets_correct_branch столкнулись с
  срабатыванием auto-fetch другого ViewModel во время подменённого fetch
  (также ошибка teardown). Отдельный повтор целиком
  tests/core/test_staged_diff.py и tests/viewmodels/test_review_followup.py
  — **40 passed, 4,45 с**. Эти падения не воспроизвелись в отдельной группе;
  их первопричины в рамках этой задачи не устранены.
- Тестовые каталоги задавались через отдельный --basetemp вне репозитория;
  для полного прогона libgit2 использовал временную глобальную конфигурацию
  с safe.directory только для этой рабочей копии. Пользовательская
  Git-конфигурация не менялась.

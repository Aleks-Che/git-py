...
### Текущий статус
- **Активный этап:** `Этап 6: Работа с удалёнными репозиториями`
- **Последнее обновление:** `2026-06-03`
- **Следующий шаг:** `Push/Pull/Fetch через pygit2.remote.Remote + async-обёртки (DEVELOPMENT_RULES.md §3 — network ops MUST be async). Управление remote (add/remove). Клонирование через диалог с провайдерами (GitHub/GitLab/BB), генерацией SSH-ключей. AuthError → колбэк на credentials. Авто-fetch по таймеру.`

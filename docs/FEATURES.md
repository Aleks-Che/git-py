...
5. **Левый мердж-коннектор защищён от fork-оверлея.** Когда коммит одновременно fork-точка (дети справа) и мердж с родителем слева, plain-`PIPE` ячейки fork-коннектора не затирают `TEE`-стыковку на лейне родителя и `HORIZONTAL_PIPE`-пересечения; полуклетка до коммита (`lane*2-1`) заливается `HORIZONTAL`, если fork занял ячейку коммита своим `TEE_RIGHT` (kilocode `a87ddecf`).

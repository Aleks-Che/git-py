...
4. **Дыры между `CROSS` и следующим fork-изгибом заполняются.** Fork-коннектор останавливается на ячейку раньше правого изгиба в расчёте на промежуточный `HORIZONTAL_PIPE`; когда предыдущий изгиб — `CROSS`, эту ячейку никто не рисовал (kilocode `9c0e4f76`, col 11).

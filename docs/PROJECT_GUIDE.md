...
│   │   ├── tracking.py             # снятие отслеживания ignored-файлов, снимок индекса
...
### Revert commit из графа

ПКМ на обычном коммите → `Revert commit` создаёт новый обратный коммит в
текущей локальной ветке. Можно отменить и старый коммит: последующие коммиты
остаются в истории. Для merge-коммита подменю `Keep parent N: SHA` выбирает
родителя, относительно которого отменяются изменения. Автор и коммиттер
берутся из настроек приложения, сообщение содержит SHA отменяемого коммита.

`MainViewModel.revert_commit()` запускает `RevertCommitCommand` через
последовательный фоновый исполнитель и `CommandProcessor`. Core проверяет
наличие локальной ветки, отсутствие незавершённой Git-операции и изменений
отслеживаемых файлов; посторонние untracked/ignored-файлы не включаются в коммит,
а коллизии с восстанавливаемыми путями отклоняются до записи.
Сводная ignored-запись каталога (например, `__pycache__/`) сама по себе не
считается коллизией: проверяются конкретные файлы назначения и их предки,
без перехода по символическим ссылкам и Windows reparse points. Это же правило
используется защитой дерева при Undo/Redo.
Undo/Redo восстанавливают сохранённые коммиты и рабочую папку с проверками
внешнего перемещения HEAD, смены ветки и локальных изменений. Redo сохраняет
результат ручного разрешения конфликтов.

При конфликте панель Revert предлагает Resolve, Continue и Abort. Последнее
сохранённое решение автоматически завершает revert; Continue принимает
решения из внешнего редактора после staging. `REVERT_HEAD` восстанавливает
панель после переоткрытия репозитория. Правая сторона редактора показывает
версию до отменяемых изменений. Abort проходит через `AbortRevertCommand`.
Прежний программный `revert()` сохраняет режим подготовки изменений без коммита.
Для merge-mainline и Abort используется Git CLI: pygit2 не предоставляет
mainline в методе `Repository.revert()`.

Регрессии: `tests/core/test_revert_commit.py`,
`tests/viewmodels/test_revert_commit.py`, `tests/ui/test_revert_commit.py`.
...
ПКМ в Unstaged/Staged → `Stop Tracking (Keep Local File)` доступен для файлов,
которые присутствуют в индексе и подпадают под действующие ignore-правила
(включая вложенные `.gitignore`, исключения `!` и `info/exclude`). При выделении
нескольких файлов пункт появляется, если подходят все выбранные пути.
`MainViewModel.stop_tracking_files()` → `StopTrackingCommand` → `core/tracking.py`
удаляет только выбранные записи индекса, сохраняя рабочие файлы. Для ранее
закоммиченных файлов в Staged появляются удаления, которые нужно закоммитить.
Файл `.gitignore` эта операция не меняет; обычный Ignore по-прежнему добавляет правило.

Операция и её Undo/Redo проходят через фоновый исполнитель при `async_enabled=True`,
с busy-индикатором и запретом повторного запуска. Undo восстанавливает сохранённые
OID/режимы выбранных записей; посторонний staging и последующие правки рабочих
файлов сохраняются. Смена HEAD/ветки, изменение выбранных записей индекса,
конфликты и пересечения путей блокируют устаревшую команду. Снятие отслеживания
отказывает, если staged-версия отличается и от HEAD, и от рабочей копии.
Регрессии: `tests/core/test_tracking.py`, `tests/viewmodels/test_stop_tracking.py`,

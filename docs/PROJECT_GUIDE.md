...
- **Цвет истории и меток:** плашки веток и popup получают цвет по имени.
  В Core имя продолжающейся ветки передаётся по цепочке коммитов с одним
  родителем независимо от колонок графа: метка новой/отстающей ветки на старом
  tip не перехватывает цвет всей истории. `main/master/dev/develop` сохраняют
  границы своих линий и приоритет при схождении; локальные refs предпочтительнее
  remote. HEAD не задаёт приоритет цвета, stash/WIP не передают владение историей.
  Через merge имя не передаётся: именованные родительские ветки сохраняют цвет.

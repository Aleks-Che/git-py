...
- Trunk-first семантика: сначала `merge-base --is-ancestor` против HEAD/main/master (local, потом `origin/`), иначе коммит, влитый в main через мердж, показывался как принадлежащий соседней feature-ветке (kilocode `95d4ab67`: origin/main содержит его как `~78^2`, а name-rev выбирал `origin/feat/cli-assistant-links~3^2`). Побочный плюс: trunk-хит ≈ 60 мс (merge-base дешевле name-rev).
- Финальная семантика — **first-parent lineage** («на какой ветке коммит был написан»): trunk считается веткой коммита только если тот лежит на её first-parent цепочке (суффикс name-rev без `^`); коммит, влитый в main мерджем, — НЕ main (так большая часть килокода не «слипается» в main). Приоритет: HEAD → main/master (local, origin) → ближайшая local → ближайшая remote. Для `95d4ab67` ответ `origin/feat/cli-assistant-links` — его настоящая ветка разработки. Оптимизации: merge-base пре-фильтр перед name-rev, memo-кэш в VM (`_branch_of_commit_cache`, сброс в `set_repository`).
- **Флаг уверенности**: `branch_of_commit` возвращает `BranchAttribution(name, certain)` (models.py). `certain=True` — коммит на first-parent цепочке ветки (структурный факт; оговорка: FF-merge неотличим от прямого коммита) → UI пишет `Branch:`. `certain=False` — достижим только через мердж, ближайший существующий ref (истинная ветка могла быть удалена) → UI пишет `Reconstructed branch:`. Все три проверенных коммита kilocode — reconstructed.

## Follow-up 4 (2026-07-23): автоподгрузка истории (infinite scroll)
- Проблема: история обрезана `graph_history_limit` (default 500) — нижний коммит kilocode 2855ebbe (13.07), а GitKraken подгружает при скролле (dfcb6235, 29.05).
- `GraphViewModel.load_more_commits()`: `history_limit += _page_size` + `refresh_graph()`; no-op при `truncated_count == 0` или без репо. `_page_size` — начальный лимит; `set_repository` сбрасывает окно на страницу.
- `GraphTableWidget`: `_on_scroll` → `_maybe_request_more_history` (порог 2 строки от низа, флаг `_loading_more` против реентерабельности); «Load more» в truncation-лейбле стал гиперссылкой (`linkActivated`).
- Тесты: VM (3: постранично, no-op без репо, сброс окна), UI (4: скролл вниз, не-срабатывание вверху, линк, остановка на полной истории). 1194 passed.

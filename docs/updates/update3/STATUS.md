...
## Follow-up L (2026-07-25): производительность UI — асинхронная панель коммита, кап рендера diff'а, async refresh при активации
- Диагностика (замеры на kilocode, headless + GUI-пробники): клик по коммиту — `branch_of_commit` ~0.2–0.6с (до 12 git-subprocess'ов) + `get_commit_changes` до 2.2с (коммит с 5MB-патчем) на UI-потоке; клик по файлу — рендер diff'а суперлинеен (50k строк = 1.8с, 5MB-патч `models-api.json` >15с в `setPlainText`); правый клик по вкладке репо — сам обработчик 0мс, но `refresh_state()` при активации окна синхронно жрал 2.3с, и клик стоял в очереди событий.
- **L1. Асинхронная загрузка деталей коммита**: `MainViewModel.request_commit_detail(sha)` — worker со своим `RepositoryManager` (info + changes + branch attribution), сигнал `commit_detail_ready`; LRU-кэш на 32 коммита (повторный клик мгновенный), прогрев `_branch_of_commit_cache`. Синхронный fallback при `async_enabled=False` (тесты). `CommitDetailPanel.show_commit` переведён на него (`_on_commit_detail_ready` отбрасывает stale-результаты).
- **L2. Асинхронный diff файла**: `MainViewModel.request_commit_file_diff(sha, path, context_lines)` + `commit_file_diff_ready`, LRU-кэш на 24 diff'а; общая логика вынесена в `core.operations.commit_file_diff_text` (worker-safe). Панель: `select_file`/`request_full_document` идут через VM; `_on_commit_file_diff_ready` различает changes-only/full по `context_lines` и фильтрует по текущему (sha, path).
- **L3. Спиннер без блокировки тулбара**: новый сигнал `MainViewModel.activity_changed` (счётчик `_activity_count`, вложенность ок) — `MainWindow` показывает тот же `QProgressBar` в статус-баре, не трогая кнопки и `_is_busy`.
- **L4. Кап рендера diff'а**: `DiffViewWidget._render` при > `_MAX_RENDERED_DIFF_LINES` (20000) строк показывает первые 20000 + баннер «diff truncated — showing N of M lines (file too large)»; полный текст хранится (toggle/copy не тронуты).
- **L5. Async refresh при активации окна**: `refresh_state()` при `async_enabled=True` делегирует в `load_repository_data()` (worker) вместо синхронного `_refresh_all_views()` — Alt-Tab больше не фризит UI, клики не стоят в очереди.

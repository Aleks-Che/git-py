...
def default_ssh_key_path() -> Path:
    """Default location for keys generated by the app."""
    return Path.home() / ".ssh" / "git-py-ed25519"


def save_ssh_key_paths(path: Path | str, private_key: str, public_key: str) -> None:
    """Activate a generated key without overwriting other current settings."""
    config = load_config(path)
    config["ssh_private_key"] = private_key
